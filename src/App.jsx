...
const addons=[["Stakeholder map","From AUD $250"],["Key messages + FAQ","From AUD $300"],["Works / project notification","From AUD $200"],["Web or EDM copy","From AUD $250"],["Additional briefing document","From AUD $300"],["Additional revision round","From AUD $150"]];
const btn="inline-flex items-center justify-center rounded-full px-5 py-3 font-black transition";
export default function App(){const[open,setOpen]=useState(false),[filter,setFilter]=useState("All"),[active,setActive]=useState(null);const filters=["All","Campaigns","Engagement","Infrastructure"];const visible=useMemo(()=>filter==="All"?projects:projects.filter(p=>p.tags.includes(filter)),[filter]);const nav=["Services","Packages","Work","About","Contact"];
return <main className="min-h-screen bg-[#f5f2ea] text-[#13251f] selection:bg-[#d5ff68]">
<header className="sticky top-0 z-50 border-b border-[#13251f]/10 bg-[#f5f2ea]/90 backdrop-blur-xl"><div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8"><a href="#top" className="text-lg font-black">ME<span className="text-[#39725c]">.</span></a><nav className="hidden items-center gap-7 md:flex">{nav.map(i=><a key={i} href={`#${i.toLowerCase()}`} className="text-sm font-medium hover:text-[#39725c]">{i}</a>)}<a target="_blank" rel="noreferrer" href={enquiryLink()} className={`${btn} bg-[#13251f] text-white hover:bg-[#39725c]`}>Discuss a project <ArrowRight className="ml-2 h-4 w-4"/></a></nav><button aria-label="Menu" onClick={()=>setOpen(!open)} className="md:hidden">{open?<X/>:<Menu/>}</button></div>{open&&<div className="px-5 py-5 md:hidden">{nav.map(i=><a onClick={()=>setOpen(false)} key={i} href={`#${i.toLowerCase()}`} className="block py-3 text-lg font-bold">{i}</a>)}</div>}</header>
<section id="top" className="mx-auto grid max-w-7xl gap-10 px-5 pb-20 pt-20 md:grid-cols-12 md:px-8 md:pb-28 md:pt-28"><div className="md:col-span-9"><motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#13251f]/15 px-4 py-2 text-xs font-bold uppercase tracking-[.14em]"><span className="h-2 w-2 rounded-full bg-[#78a944]"/>Available for freelance & contract projects</motion.div><motion.h1 initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} className="max-w-5xl text-6xl font-black leading-[.91] tracking-[-.065em] sm:text-7xl md:text-[108px]">Clear communication for projects that <span className="text-[#39725c]">matter.</span></motion.h1></div><div className="flex flex-col justify-end md:col-span-3"><p className="mb-6 text-lg leading-relaxed text-[#13251f]/70">I'm Morgan Evans. I help organisations turn complex projects, stakeholder needs and technical information into communications people can understand and act on.</p><a target="_blank" rel="noreferrer" href={enquiryLink()} className={`${btn} mb-4 bg-[#d5ff68] text-[#13251f] hover:bg-[#c7ef58]`}>Tell me about your project <ArrowRight className="ml-2 h-4 w-4"/></a><a href="#packages" className="text-center text-sm font-bold underline underline-offset-4">View packages and rates</a></div></section>
...
<section id="contact" className="mx-3 mb-3 rounded-[2.5rem] bg-[#39725c] px-5 py-20 text-white md:px-8"><div className="mx-auto grid max-w-7xl gap-10 md:grid-cols-12"><div className="md:col-span-8"><p className="mb-5 text-xs font-black uppercase tracking-[.18em] text-[#d5ff68]">Let's work together</p><h2 className="text-5xl font-black md:text-8xl">Let's make your project easier to communicate.</h2></div><div className="flex flex-col justify-end gap-3 md:col-span-4"><a target="_blank" rel="noreferrer" href={enquiryLink()} className={`${btn} bg-[#d5ff68] text-[#13251f]`}><Mail className="mr-2 h-4 w-4"/>Email Morgan</a><a target="_blank" rel="noreferrer" href={LINKEDIN} className={`${btn} border border-white/30 text-white`}>LinkedIn <ExternalLink className="ml-2 h-4 w-4"/></a></div></div></section>
<footer className="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-9 text-xs text-[#13251f]/55 md:flex-row md:justify-between md:px-8"><div>© 2026 Morgan Evans · Communications & Stakeholder Engagement</div><div className="flex gap-5"><a target="_blank" rel="noreferrer" href={enquiryLink()}>Email</a><a target="_blank" rel="noreferrer" href={LINKEDIN}>LinkedIn</a><a href="#packages">Rates</a><a href="#top">Back to top</a></div></footer>
{active&&<div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#13251f]/60 p-5 backdrop-blur-sm" onClick={()=>setActive(null)}><div onClick={e=>e.stopPropagation()} className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] bg-[#f5f2ea] p-8"><div className="flex justify-between"><div><p className="text-xs font-black uppercase text-[#39725c]">{active.type}</p><h2 className="mt-2 text-4xl font-black">{active.title}</h2></div><button onClick={()=>setActive(null)}><X/></button></div><div className="mt-8 grid gap-8 md:grid-cols-2"><div><b>The challenge</b><p className="mt-2 text-[#13251f]/70">{active.challenge}</p></div><div><b>My role</b><p className="mt-2 text-[#13251f]/70">{active.role}</p></div><div className="rounded-2xl bg-[#dce5d6] p-6"><b>What I delivered</b>{active.delivered.map(x=><div key={x} className="mt-3 flex gap-2"><CheckCircle2 className="h-5 w-5 text-[#39725c]"/>{x}</div>)}</div><div className="rounded-2xl bg-white p-6"><b>Outcome</b><p className="mt-2 text-[#13251f]/70">{active.outcome}</p></div></div></div></div>}
</main>}
